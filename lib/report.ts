...
import { describeAction, formatMoney, monitoringSummary, type MonitoringSummary } from "./optimizer.ts";
...
export interface ClientReport {
  ...
  /** What Pulso watched during the period, so a week without changes still shows the work done. */
...
export function buildClientReport(workspace: WorkspaceData, organizationId: string, now: Date): ClientReport | null {
  ...
  return {
    ...
    monitoring: monitoringSummary(workspace, organizationId, now, REPORT_DAYS),
    agentWork: {
...
/** True when nothing needed attention: the message for a calm week is good news, not silence. */
export function isQuietPeriod(report: ClientReport): boolean {
  return !report.agentWork.executed && !report.agentWork.blocked && !report.monitoring.anomalies;
}

/** One-sentence account of the monitoring work, or null when no run was recorded yet. */
export function monitoringSentence(report: ClientReport): string | null {
  const { runs, adsWatched, campaignsWatched, pacingChecks } = report.monitoring;
  if (!runs) return null;
  const plural = (count: number, one: string, many: string) => `${count} ${count === 1 ? one : many}`;
  return `Pulso revisó tu cuenta ${plural(runs, "vez", "veces")}, vigiló ${plural(adsWatched, "anuncio", "anuncios")} en ${plural(campaignsWatched, "campaña", "campañas")}, verificó el ritmo de gasto ${plural(pacingChecks, "vez", "veces")} y cuidó tu límite mensual de ${formatMoney(report.organization.monthlyLimit)}.`;
}

export function outcomeSentence(report: ClientReport): string {
  if (isQuietPeriod(report)) return "Todo funcionó dentro de tus metas: no hizo falta intervenir.";
  const { anomalies } = report.monitoring;
  const { executed, blocked } = report.agentWork;
  const plural = (count: number, one: string, many: string) => `${count} ${count === 1 ? one : many}`;
  return `Detectó ${plural(anomalies, "señal de riesgo", "señales de riesgo")}, aplicó ${plural(executed, "cambio", "cambios")} y frenó ${plural(blocked, "cambio que rompía", "cambios que rompían")} tus límites.`;
}
...
export function renderReportEmail(report: ClientReport, reportUrl: string): { subject: string; html: string } {
  ...
  const monitoring = monitoringSentence(report);
...
${monitoring ? `<div style="margin:16px 0;padding:14px 16px;border-radius:8px;background:#f1fbf6;border:1px solid #dcefe6"><div style="font-size:14px;font-weight:700;color:#11845f;margin-bottom:4px">Esta semana</div><div style="font-size:14px;line-height:1.5;color:#33343d">${e(monitoring)} ${e(outcomeSentence(report))}</div></div>` : ""}
<p style="font-size:14px;line-height:1.5;color:#585a66;margin:16px 0">En el mes llevan ${e(formatMoney(report.month.spend))} de un límite de ${e(formatMoney(report.organization.monthlyLimit))} (${Math.round(report.month.budgetUsed * 100)}%). Cambios de los agentes esta semana: ${work.budgetChanges} de presupuesto, ${work.pauses} pausas y ${work.resumes} reactivaciones.</p>
